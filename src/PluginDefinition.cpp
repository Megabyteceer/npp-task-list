						  ...
						  DWORD /*dwTime*/) // current system time 
{
	...
	int length = static_cast<int>(::SendMessage(curScintilla, SCI_GETLENGTH, 0, 0));
	...
	for (int keyword_index = 0; keyword_index < keyword_count; keyword_index++) {
		...
		assert(static_cast<int>(strlen(keyword)) < k_max_keyword_length);
...
// Dockable Dialog for Task List
//
// You have to create the dialog by inherented DockingDlgInterface class in order to make your dialog dockable
